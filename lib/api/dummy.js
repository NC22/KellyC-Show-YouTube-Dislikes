...
KellyShowRate.apiController['API_ID'].onGetYDataReady = function(handler, requestBg, response, onReady) {
    
    // use response without modification 
    
    return true; // async mode - onReady will be called by native controller
}
...
KellyShowRate.apiController['API_ID'].onPrepareGetRatingRequestStart = function(handler, requestCfg, onReady) {
     ...
     return true; // sync mode - onReady will be called by native controller
}

/*
    Calls before action request. Must return to onReady params array if action request supported
    
    var requestContext = {videoId : videoId, uuid : getUserId(), type : type, undo : undo};
    
    on success return requestCfg
...
KellyShowRate.apiController['API_ID'].onGetActionDataReady = function(handler, response) {
