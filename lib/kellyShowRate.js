...
function KellyShowRate(cfg) {
    ...
    function updateRatioWidth() {
        ...
        if (!isMobile() && handler.ratioBar && handler.buttonsWraper && handler.buttonsWraper.children.length > 1) {
    ...
    function parseYTPage(docRawText) {
        ...
        var publicData = {likes : false, dislikes : false, count : false, rating : false, utDislikes : false, source : 'Rating'}, pairs = {LIKE : 'likes', DISLIKE : 'dislikes'};
    ...
    function updatePageStateByDriver(driver, force) {
        ...
        if (!driver) driver = 0;
        handler.log('[updatePageStateByDriver] update [' + driver + ']', true); 
        ...
        handler.ytRequest = getYTData(driverUrl[driver], function(ydata) {
           ...
           if (!showYData(ydata, 'getYTData.regular')) {
                ...
                if (driver <= driverUrl.length-1) updatePageStateByDriver(driver, true);
                    ...
                    setTimeout(function() { updatePageStateByAR(false, true); }, handler.requestsCfg.failTimeoutApi);
                }
           }
        }); 
    }
    ...
    function updatePageStateByAR(attempt, force) {
        ...
        handler.log('[updatePageStateByAR] update [' + attempt + '/' + maxAttempts + ']', true); 
        var videoId = prepareRequestStart(force);
        if (!videoId) return;
        ...
        handler.ytRequest = getYTData('https://www.youtube.com/watch?v=' + videoId, function(ydata, error) {
            ...
            if (error) {
                ...
                if (error != 'dataParserError' && attempt+1 <= maxAttempts) updatePageStateByAR(attempt+1, true);
                    ...
                    setTimeout(function() { updatePageStateByDriver(false, true); }, handler.requestsCfg.failTimeoutApi);
                }
            }                    
        });
    }
    ...
    this.updatePageStateDelayed = function(d) {
        ...
        KellyStorage.load(function(cfg) {
            ...
            if (isMobile()) {
                ...
                handler.observer = new MutationObserver(function(mutations) {
                    ...
                    for (var i = 0; i < mutations.length; i++) {
                        ...
                        if (mutations[i].target.classList.contains('slim-video-action-bar-actions') &&
                            ...
                            resetNavigation();  
                            handler.updatePageStateDelayed(1000);
                            return;
                        } 
                    }                    
                });
                
                handler.observer.observe(document.getElementById('app'), {childList: true, subtree: true});
                handler.updatePageStateDelayed();
            ...
            document.addEventListener('click', function (e) {
                if (handler.getTooltip().isChild(e.target, handler.buttonsWraper.children[0]) || 
                    ...
                    // drop cache if needed, take cache currently if exist    
