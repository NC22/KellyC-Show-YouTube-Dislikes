...
function KellyShowRate(cfg) {
    ...
    function updateRatio(ydata) {
        ...
        var showTip = function (e, stick) { 
           ...
           handler.getTooltip().setMessage('<div class="' + handler.baseClass + '-count">' + KellyTools.dFormat(ydata.likes) + ' / ' + KellyTools.dFormat(ydata.dislikes) + '</div>' + additionNotice);
    ...
    */    
    ...
    function prepareActionRequestStart(apiId, type, undo, onReady) {
         ...
         if (!requestContext.uuid || !requestContext.videoId || requestContext.type == 'neutral' || !browsingLog[requestContext.videoId]) {
             onReady(false, 'BAD input data [l:' + requestContext.uuid + ' |v: ' + requestContext.videoId + ' |s: ' + requestContext.type + ' | ' + (browsingLog[requestContext.videoId] ? 'history OK' : 'history NO'));
             return;
         }
    ...
    function actionRequest(type) {
        ...
        if (!type) type = getRatingState();
        
        if (type == 'unkonwn') return false;
        ...
        for (var apiId in KellyStorage.apis) actionRequestInitForApi(apiId, type);
    }
    ...
    function updateCounter(type, counterEl, val) {
        ...
        if (val === false) {
            ...
            KellyTools.setHTMLData(holder, KellyTools.nFormat(val));
        }
    }
    ...
    function showYData(ydata, callerId) {
        ...
        */
        ...
        var clearButtonCounter = function(el) {
            if (!el) return;
            var num = parseInt(el.innerText.replace(/[^0-9]/g,''));
            if (!isNaN(num)) el.innerText = '';
        }
    ...
    this.init = function() {
        ...
        KellyStorage.load(function(cfg) {
            ...
            initCss();
            handler.log(isMobile() ? '[Mobile]' : '[Desktop] version controller', true);
            ...
            document.addEventListener('click', function (e) {
                ...
                if (handler.getTooltip().isChild(e.target, handler.buttonsWraper.children[0]) || 
                    ...
                    if (lastVideoId && browsingLog[lastVideoId]) actionRequest();
