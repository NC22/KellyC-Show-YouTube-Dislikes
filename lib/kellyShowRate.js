...
function KellyShowRate(cfg) {
    ...
    function updateRatio(ydata) {
        ...
        if (handler.cfg.ratioLikeColor) likeEl.style.backgroundColor = handler.cfg.ratioLikeColor;
        if (handler.cfg.ratioDislikeColor) dlikeEl.style.backgroundColor = handler.cfg.ratioDislikeColor;
            ...
            if (isMobile()) {
                ...
                handler.observer = new MutationObserver(function(mutations) {
                    ...
                    for (var i = 0; i < mutations.length; i++) {
                        ...
                        if (mutations[i].type == 'childList' && mutations[i].target.classList.contains('slim-video-information-content')) {
                            
                            handler.updatePageStateDelayed(200, 'video-header');
                            return;
                            
                        } else if (mutations[i].type == 'childList' &&  mutations[i].target.classList.contains('slim-video-action-bar-actions') &&
