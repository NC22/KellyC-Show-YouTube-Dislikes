...
var KellyCOptions = new Object(); 
    ...
    KellyCOptions.addDataSources = function() {
         ...
         for (var i = 0; i < handler.cfg.apis.order.length; i++) {
            ...
            html += '<label for="data-source-sync-' + key + '"><input type="checkbox" id="data-source-sync-' + key +'" data-target="' + key +'" ' + (handler.cfg.apis.cfg[key].syncData ? 'checked' : '') + '>'  + handler.getLoc('datasource_send') + '</label>';
            ...
            html += '<input type="hidden" id="' + key + '-ratioLikeColor" value="' + KellyTools.val(handler.cfg.apis.cfg[key].ratioLikeColor) + '">\
                     ...
                     <input type="hidden" id="' + key + '-ratioLoadingColor" value="' + KellyTools.val(handler.cfg.apis.cfg[key].ratioLoadingColor) + '">';
                     
            if (handler.picker) html += '<div class="data-source-colorpicker"></div>';
                
            html += '<div class="data-source-name">' + KellyStorage.apis[key].name + '</div>';
            ...
            if (KellyStorage.apis[key].desc) {
                html += '<div class="data-source-desc" id="data-source-desc-' + key + '">' + KellyStorage.apis[key].desc + '</div>';
            }       
        ...
        for (var i = mInputs.length-1; i >= 0; i--) {
            mInputs[i].onclick = function() { 
                ...
                var index = handler.cfg.apis.order.indexOf(key);
                ...
                var switchKey = handler.cfg.apis.order[switchIndex]; 
                
                handler.cfg.apis.order[index] = switchKey;
                handler.cfg.apis.order[switchIndex] = key;
                
                var itemEl = document.getElementById('data-source-' + key);
                var switchItemEl = document.getElementById('data-source-' + switchKey);
    ...
    KellyCOptions.addColorRing = function() {
        ...
        picker.initInputs = function(toDefault) {
            ...
            for (var i = mInputs.length-1; i >= 0; i--) {
                ...
                if (toDefault && picker.clearInputForDefault) targetInput.value = '';
            }
        }
